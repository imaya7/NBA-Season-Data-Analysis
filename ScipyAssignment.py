...
# Extract regression coefficients for line of best fit equation
slope = reg.coef_[0]
intercept = reg.intercept_
printOutput(f"\n3. Linear Regression Analysis:")
printOutput(f"Line of Best Fit Equation: 3P Accuracy = {slope:.12f} × Year + {intercept:.12f}")
printOutput(f"R-squared: {reg.score(X, y):.12f}")

# Add predicted values to dataframe
playerData.loc[:, '3pAccuracyPred'] = reg.predict(X)
...
predictedAccuracies = reg.predict(seasonsRange.reshape(-1, 1))

# Integrate using the trapezoidal rule
averagePredictedAccuracy = np.trapz(predictedAccuracies, seasonsRange) / (lastSeason - firstSeason)
...
interpolatedValues = reg.predict(np.array(missingSeasons).reshape(-1, 1))
...
yearLine = np.linspace(yearMin, yearMax, 100).reshape(-1, 1)
accuracyLine = reg.predict(yearLine)
